...
, m_datacostIndividual( NULL )
, m_smoothcostIndividual( NULL )
, m_labelingDataCosts(new EnergyTermType[nSites])
, m_labelCounts(new SiteID[nLabels])
, m_activeLabelCounts( new SiteID[m_num_labels] )
, m_labelcostsAll( NULL )
, m_labelcostsByLabel( NULL )
, m_labelcostCount(0)
, m_labelingInfoDirty(true)
, m_verbosity(0)
, m_datacostFn( NULL )
, m_smoothcostFn( NULL )
, m_beforeExpansionEnergy(0) // Uninitialized in original version => 0 is guess for proper initialization
, m_numNeighbors( NULL )
, m_numNeighborsTotal(0)
, m_giveSmoothEnergyInternal( NULL )
, m_queryActiveSitesExpansion(&GCoptimization::queryActiveSitesExpansion<DataCostFnFromArray>)
, m_setupDataCostsExpansion( NULL )
, m_setupSmoothCostsExpansion( NULL )
, m_setupDataCostsSwap( NULL )
, m_setupSmoothCostsSwap( NULL )
, m_applyNewLabeling( NULL )
, m_updateLabelingDataCosts( NULL )
, m_datacostFnDelete( NULL )
, m_smoothcostFnDelete( NULL )
